...
// Require QA to be executed with MCP-Playwright. If enabled, GREEN is only
// accepted when runner === 'mcp'. Default: enabled.
const QA_REQUIRE_MCP = String(process.env.QA_REQUIRE_MCP || "1") !== "0";
...
async function main() {
  ...
  allowFailure: true,
    ...
    // Accept only if GREEN and (runner is MCP or gating disabled)
